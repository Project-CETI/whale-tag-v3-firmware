...
static int priv__oriented_upright(void) {
    ...
    return (d_pitch_norm < FLOAT_DETECT_TARGET_ACCURACY_RAD) && (d_roll_norm < FLOAT_DETECT_TARGET_ACCURACY_RAD);
}
...
int float_detection_is_floating(void) {
    return priv__oriented_upright() && (rtc_get_epoch_s() - s_float_start_time_s > FLOAT_DETECT_HOLD_TIME_S);
}
