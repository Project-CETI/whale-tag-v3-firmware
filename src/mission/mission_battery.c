...
static inline int priv__mission_battery_is_tracking_enabled(void) {
    return (tag_config.hw_config.bms.available && tag_config.battery.enabled);
}
...
void mission_battery_init(void) {
    if (!priv__mission_battery_is_tracking_enabled() || !tag_config.mission.low_power_release.enabled) {
        return;
    }
...
int mission_battery_is_in_error(void) {
    return (priv__mission_battery_is_tracking_enabled() && (s_battery_consecutive_error_count + 1 >= BATTERY_ERROR_COUNT_THRESHOLD));
}