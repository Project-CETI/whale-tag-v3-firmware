...
static void priv__write_static_hardware_config(void) {
    ...
    if (tag_config.hw_config.vhf_pinger.available) {
        offset += snprintf((char *)&buffer[offset], sizeof(buffer) - offset,
                           ...
                           (HW_VERSION == HW_VERSION_3_2_0) ? "true" : "false", tag_config.hw_config.vhf_pinger.carrier_frequency_mhz);
    }
    ...
    if (tag_config.hw_config.audio.available) {
        if (!tag_config.audio.enabled) {
            ...
            offset += snprintf((char *)&buffer[offset], sizeof(buffer) - offset,
                               ...
                               "      - enabled: %s\n"
                               "      - enabled: %s\n",
                               tag_config.audio.bitdepth, (tag_config.audio.filter_type == AUDIO_FILTER_WIDEBAND) ? "wideband" : (tag_config.audio.filter_type == AUDIO_FILTER_SINC) ? "sinc"
                                                                                                                                                                                     : "null",
                               (tag_config.audio.priority == AUDIO_PRIORITIZE_POWER)   ? "power"
                               : (tag_config.audio.priority == AUDIO_PRIORITIZE_NOISE) ? "sinc"
                                                                                       : "null",
                               tag_config.audio.samplerate_sps, (tag_config.audio.channel_enabled[0]) ? "true" : "false", (tag_config.audio.channel_enabled[1]) ? "true" : "false", (tag_config.audio.channel_enabled[2]) ? "true" : "false", (tag_config.audio.channel_enabled[3]) ? "true" : "false");
        }
    }
...
static void priv__write_static_mission_config(void) {
    ...
    if (!tag_config.mission.timer_release.enabled) {
        ...
        offset += snprintf((char *)&buffer[offset], sizeof(buffer) - offset,
                           ...
                           "    hours: %d\n"
                           "    minutes: %d\n",
                           tag_config.mission.timer_release.hours, tag_config.mission.timer_release.minutes);
    }
    ...
    if (!tag_config.mission.time_of_day_release_utc.enabled) {
        ...
        offset += snprintf((char *)&buffer[offset], sizeof(buffer) - offset,
                           ...
                           "    enabled: true\n"
                           "    time: %02d:%02d UTC\n",
                           tag_config.mission.time_of_day_release_utc.hour, tag_config.mission.time_of_day_release_utc.minute);
    }
...
void metadata_log_file_creation(char *filename, DataType data_type, DataFormat format, uint16_t version) {
    ...
    char buffer[2048];
    offset += snprintf((char *)&buffer[offset], sizeof(buffer) - offset,
                       "    - file: { filename: %s, type: %s, format: %s, version: %d }\n", filename, data_type_str[data_type], format_str[format], version);
...
void metadata_create(char *mission_directory) {
    ...
    offset += snprintf((char *)&buffer[offset], sizeof(buffer) - offset,
                       ...
                       "id: %s\n"
                       "timestamp: %lld\n",
                       tag_config.hostname, rtc_get_epoch_s());
