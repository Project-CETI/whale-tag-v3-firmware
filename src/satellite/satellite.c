...
#define __SATELLITE_GET_COMMAND(cmd, dst, length, capacity) do {                        \
    ...
    if (NULL == end_ptr) { return 4; }													\
    size_t len = end_ptr - start_ptr;                                                   \
    if (len > capacity) { return 5; }													\
...
/// @note  it sucks that metaprogramming isn't a thing in C (MSH) 
#define __SATELLITE_SET_COMMAND(cmd, value_array, length) do { \
