...
bool PREVIPASS_estimate_next_pass_with_status(
    ...
    float xBeaconCartesian = cosf(config->beaconLatitude * C_MATH_DEG_TO_RAD) * cosf(config->beaconLongitude * C_MATH_DEG_TO_RAD);
    float yBeaconCartesian = cosf(config->beaconLatitude * C_MATH_DEG_TO_RAD) * sinf(config->beaconLongitude * C_MATH_DEG_TO_RAD);
    ...
    if (aopTable->downlinkStatus == SAT_DNLK_OFF && aopTable->uplinkStatus == SAT_UPLK_OFF)
        return true;
    ...
    float meanMotionDriftRevPerSec = -.75f * semiMajorAxisDriftKmPerDay / aopTable->semiMajorAxisKm / 86400.0f * C_MATH_TWO_PI * numberOfRevSinceBulletin;
    ...
    while (secondsSinceBulletin < computationEndSecondsSinceBulletin) {
        ...
        if ((distance2 < visibilityMinDistance2) &&
            ...
            float elevation = (aopTable->semiMajorAxisKm * sinf(v)) / sqrtf(C_MATH_EARTH_RADIUS * C_MATH_EARTH_RADIUS + aopTable->semiMajorAxisKm * aopTable->semiMajorAxisKm - 2 * C_MATH_EARTH_RADIUS * aopTable->semiMajorAxisKm * cosf(v));
        ...
        if (isInPass == 1) {
            ...
            if (elevationMax <= config->maxElevation &&
                ...
                uint32_t timeMarginSec =
                    (uint32_t)(config->timeMarginMinPer6months * (uint32_t)60 * (uint32_t)secondsSinceBulletin / (86400 * 365 / 2.f));
                ...
                ppItem.epoch = (uint32_t)bullSec90 + (uint32_t)secondsSinceBulletin - (uint32_t)passDurationSec - (uint32_t)timeMarginSec + (uint32_t)EPOCH_90_TO_70_OFFSET;
        ...
        if ((distance2 >= 4 * visibilityMinDistance2) && (distance2 <= (16 * visibilityMinDistance2)))
