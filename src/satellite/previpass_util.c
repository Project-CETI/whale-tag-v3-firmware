...
)
{
	...
	uint32_t numberOfYears = (uint32_t)dateTime->year - 1990;

	// Number of leap years since 1990
	uint32_t numberOfLeapYears = ((uint32_t)dateTime->year - 1 - 1900) / 4 - 22;
...
)
{
	...
	dateTime->hour = (uint32_t)temp / 3600;
	temp -= (uint32_t)dateTime->hour * 3600;
	dateTime->minute = (uint32_t)temp / 60;
	temp -= (uint32_t)dateTime->minute * 60;
	dateTime->second = temp;

	// Number of years since 1990
	uint16_t numberOfYears = ((uint32_t)numberOfDays + 0.5f) / 365.25f;

	dateTime->year = numberOfYears + 1990;

	// Number of leap years since 1990
	uint8_t numberOfLeapYears = (dateTime->year - 1 - 1900) / 4 - 22;

	// Leap year flag (0: leap, 1: non leap)
	uint8_t isLeapYear = MIN(dateTime->year % 4, 1);

	// Number of day since start of year
	numberOfDays -= (uint32_t)numberOfYears * 365 + (uint32_t)numberOfLeapYears - 1;
