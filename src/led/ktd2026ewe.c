...
static HAL_StatusTypeDef priv__ktd2026ewe_write(uint8_t register_address, uint8_t value) {
...
HAL_StatusTypeDef ktd2026ewe_set_led_mode(uint8_t led_index, ktd2026ewe_LedMode mode) {
    ...
    return priv__ktd2026ewe_write(KTD2026EWE_REG_CHANNEL_CONTROL, s_self.ch_control);
}
...
HAL_StatusTypeDef ktd2026ewe_set_led_current(uint8_t led_index, float current_mA) {
    ...
    return priv__ktd2026ewe_write(KTD2026EWE_REG_IOUT_1 + led_index, s_self.current[led_index]);
}
...
HAL_StatusTypeDef ktd2026ewe_set_period_s(float period_s) {
    ...
    return priv__ktd2026ewe_write(KTD2026EWE_REG_FLASH_PERIOD, s_self.period);
}

HAL_StatusTypeDef ktd2026ewe_set_pwm1_duty(float duty) {
    s_self.pwm_duty[0] = (uint8_t)(duty * 256.0);
    return priv__ktd2026ewe_write(KTD2026EWE_REG_DUTY_1, s_self.pwm_duty[0]);
}

HAL_StatusTypeDef ktd2026ewe_set_pwm2_duty(float duty) {
    s_self.pwm_duty[1] = (uint8_t)(duty * 256.0);
    return priv__ktd2026ewe_write(KTD2026EWE_REG_DUTY_2, s_self.pwm_duty[1]);
}

HAL_StatusTypeDef ktd2026ewe_reset(void) {
    HAL_StatusTypeDef result = priv__ktd2026ewe_write(KTD2026EWE_REG_RESET, 0b111);
    ...
    // move pwm2 to slot 3
    s_self.slot = 0b010;
    return priv__ktd2026ewe_write(KTD2026EWE_REG_RESET, s_self.slot);
